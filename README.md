...
- **Серверные функции**: Vercel API Routes
...
Для автоматической отправки запланированных постов используется сервис [cron-job.org](https://cron-job.org/en/), который позволяет настроить запуск до 60 раз в час (каждую минуту).
...
История выполнения доступна в интерфейсе cron-job.org с подробной информацией о каждом запуске.
